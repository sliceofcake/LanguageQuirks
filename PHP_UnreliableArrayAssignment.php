...
// Spoiler : If you ever have a living variable that references part of an array, that part of the array will be ~referenced copied~ when you use equals-sign assignment. Note that as soon as that variable is unset($_), things will return to normal. To see this, try adding the code line "unset($unused);" immediately after the line where $unused is assigned. Now the copies will remain unchanged, as expected. The conclusion is that you shouldn't trust equals-sign assignment to copy an array, you should always use a custom deep-copy function when you need to copy an array.
